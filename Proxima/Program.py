from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QGridLayout, QLabel, QPushButton, QGraphicsView, QGraphicsScene, QTextEdit, QLineEdit, QMessageBox, QFileDialog, QInputDialog, QColorDialog, QSlider, QSizePolicy, QTextBrowser
from PyQt5.QtCore import Qt, QPoint, QTimer, QDate, QTime, QDir, QUrl, QDateTime
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Set the background color of the main window to light grey
        ...
        grid_layout.setRowStretch(0, 1)  # Full weightage to row 0
        grid_layout.setRowStretch(1, 0)  # Low weightage to row 1

        # Create the left center widget
        ...
        # Set layout margins to include 10px offset from the left edge
        ...
        # Create the bottom center widget
        ...
        # Create the inner widgets for the bottom center widget
        for i in range(3):
            ...
            if i == 1:  # Adjust width of middle inner widget
                inner_widget.setFixedSize(325, 75)  # Set to 325 pixels wide

                # Add QLineEdit and QPushButton to the middle inner widget
                ...
                icon_ = os.path.abspath("Icons/searchButton.png")
                icon = QIcon(icon_)
                ...
                if i == 0:  # Add time and date labels to the leftmost inner widget
                    ...
                    # Set up a timer to update the time and date
                    timer = QTimer(self)
                    timer.timeout.connect(self.updateDateTime)
                    timer.start(1000)  # Update every second

                    self.updateDateTime()  # Initial call to set the time and date immediately

                if i == 2:  # Add a red button to the rightmost inner widget
                    shutdown_button = QPushButton()
                    icon_ = os.path.abspath("Icons/powerButton.png")
                    icon = QIcon(icon_)
                    ...
                    shutdown_button.clicked.connect(self.close)  # Connect button click to close method

                    inner_layout = QGridLayout()
                    inner_widget.setLayout(inner_layout)
                    inner_layout.addWidget(shutdown_button, 0, 0, Qt.AlignCenter)

                    # Comment on how to shut down a Linux machine
                    # To shut down a Linux machine, you can use the following command:
    ...
    def addButtons(self, layout):
        ...
        for key, value in self.buttonElements.items():
            ...
            button.setStyleSheet("background-color: lightgrey; border: none;")  # Red background and no border
    ...
    def runApp(self, width, height, appName, app):
        self.mainWindow = Window(width, height, appName, app)
...
class Window(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint)  # Remove window borders
